...
@section('content')
    ...
    <x-content>
        ...
        <form class="px-2" method="POST" action="/clients/{{ $client->id }}">
            @csrf
            @method('PATCH')
            <div class="mb-6">
                ...
                <input type="text" id="company" name="company"
                    ...
                    @else
                        border-gray-300
                    @enderror"
                    placeholder="Gerlach Inc" value="{{ old('company') ?? $client->company }}" required>
                @error('company')
            ...
            <div class="mb-6">
                ...
                <input type="number" id="vat" name="vat" min="1000" max="99999"
                    ...
                    @else
                        border-gray-300
                    @enderror"
                    placeholder="00000" value="{{ old('vat') ?? $client->vat }}" required>
                @error('vat')
                    <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">
                        {{ $message }}
                    </p>
            ...
            <div class="grid md:grid-cols-2 md:gap-6 mb-6">
                <div class="mb-1 relative w-full group">
                    ...
                    @else
                        border-gray-300
                    @enderror"
                        placeholder="33369 Angus Shores Suite 622" value="{{ old('street_address') ?? $client->address->street_address }}" required>
                    @error('street_address')
                        <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">
                            {{ $message }}
                        </p>
                ...
                <div class="mb-1 relative w-full group">
                    ...
                    @else
                        border-gray-300
                    @enderror"
                        placeholder="West Elbert" value="{{ old('city') ?? $client->address->city }}" required>
                    @error('city')
                        <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">
                            {{ $message }}
                        </p>
                ...
                <div class="mb-1 relative w-full group">
                    ...
                    @else
                        border-gray-300
                    @enderror"
                        placeholder="California" value="{{ old('state') ?? $client->address->state }}" required>
                    @error('state')
                        <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">
                            {{ $message }}
                        </p>
                ...
                <div class="mb-1 relative w-full group">
                    ...
                    @else
                        border-gray-300
                    @enderror"
                        placeholder="12345 or 12345-6789" value="{{ old('zip_code') ?? $client->address->zip_code }}" required>
                    @error('zip_code')
                        <p id="filled_error_help" class="mt-2 text-xs text-red-600 dark:text-red-400">
                            {{ $message }}
                        </p>
